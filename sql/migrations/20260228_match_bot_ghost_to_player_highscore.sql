-- Pick bot ghost timelines near the real player's highscore for the same minigame.

CREATE OR REPLACE FUNCTION public.pick_ghost_timeline(p_player_id text, p_game_type text) RETURNS jsonb
...
BEGIN
    SELECT mode, game_type, player1_id, player2_id
    INTO v_mode, v_current_type, v_p1, v_p2
    FROM game_sessions
    WHERE id = p_room_id;
...
GRANT EXECUTE ON FUNCTION public.pick_ghost_timeline(text, text) TO postgres, anon, authenticated, service_role;
