  ...
  return (
    <div className="fixed inset-0 z-[120] bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-gray-900 p-5 shadow-2xl">
        <h3 className="text-lg font-bold text-white">{t('report.title', '신고')}</h3>
        ...
        <textarea
          ...
          className="mt-3 h-32 w-full resize-none rounded-xl border border-white/15 bg-black/25 p-3 text-sm text-white outline-none focus:border-red-400/70"
          maxLength={MAX_REASON_LENGTH}
        />

        <div className="mt-1 text-right text-xs text-gray-400">{reason.length}/{MAX_REASON_LENGTH}</div>

        <div className="mt-4 flex gap-2">
          <button
            onClick={onClose}
            disabled={isSubmitting}
            className="flex-1 rounded-xl border border-white/15 px-4 py-2.5 text-sm font-semibold text-gray-200 hover:bg-white/5 disabled:opacity-50"
          >
            {t('common.cancel')}
          </button>
          <button
