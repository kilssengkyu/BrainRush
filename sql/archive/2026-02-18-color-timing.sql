-- Color Timing migration (replace Timing Bar in random/practice/highscore usage)

CREATE OR REPLACE FUNCTION public.get_game_duration(p_game_type text) RETURNS integer
...
CREATE OR REPLACE FUNCTION public.stat_increments(p_game_type text) RETURNS TABLE(speed integer, memory integer, judgment integer, calculation integer, accuracy integer, observation integer)
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
ADD CONSTRAINT game_sessions_game_type_check CHECK (
  game_type = ANY (
    ARRAY[
      'RPS'::text, 'NUMBER'::text, 'MATH'::text, 'TEN'::text, 'COLOR'::text,
      'MEMORY'::text, 'SEQUENCE'::text, 'SEQUENCE_NORMAL'::text, 'LARGEST'::text,
      'PAIR'::text, 'UPDOWN'::text, 'SLIDER'::text, 'ARROW'::text, 'NUMBER_DESC'::text,
      'BLANK'::text, 'OPERATOR'::text, 'LADDER'::text, 'TAP_COLOR'::text, 'AIM'::text,
      'MOST_COLOR'::text, 'SORTING'::text, 'SPY'::text, 'PATH'::text, 'BLIND_PATH'::text,
      'BALLS'::text, 'CATCH_COLOR'::text, 'TIMING_BAR'::text, 'COLOR_TIMING'::text,
      'STAIRWAY'::text
    ]
  )
);
