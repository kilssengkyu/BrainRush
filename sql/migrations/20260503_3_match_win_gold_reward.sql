-- Give 5 gold to the real winner of item(normal)/rank matches.
-- This replaces finish_game with the current production logic plus the win gold reward.
...
BEGIN
    ...
    IF v_winner IS NOT NULL AND NOT v_is_draw THEN
        IF v_session.mode = 'rank' THEN
             -- RANK MODE: include bot MMR from bot_profiles for realistic deltas
             ...
             IF v_p1_is_real OR v_p2_is_real THEN
                 ...
                 ELSE
                     SELECT mmr INTO v_p1_mmr FROM bot_profiles WHERE id = v_session.player1_id;
                 END IF;

                 IF v_p2_is_real THEN
                     SELECT mmr INTO v_p2_mmr FROM profiles WHERE id = v_session.player2_id::uuid;
                 ELSE
                     SELECT mmr INTO v_p2_mmr FROM bot_profiles WHERE id = v_session.player2_id;
                 END IF;
