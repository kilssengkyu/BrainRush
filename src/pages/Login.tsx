...
const Login = () => {
    ...
    return (
        <div className={`min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden bg-slate-50 dark:bg-gray-900 text-slate-900 dark:text-white`}>
            {/* Background Effects */}
            <div className={`absolute top-0 left-0 w-full h-full pointer-events-none ${themeMode === 'light'
                ? 'bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-white via-slate-100 to-slate-200'
                : 'bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-blue-900/40 via-gray-900 to-black'
                }`} />
            ...
            <motion.div
                ...
                className="z-10 w-full max-w-md bg-white dark:bg-gray-800/50 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl relative"
            >
                {/* Back Button */}
                <button
                    onClick={handleBack}
                ...
                >
                    <ArrowLeft size={24} className="text-slate-500 dark:text-gray-400" />
                </button>
                ...
                <div className="text-center mb-10 mt-4">
                    ...
                    <p className="text-slate-500 dark:text-gray-400">{t('menu.login')}</p>
                </div>
            ...
            {banModal && (
                <div className="fixed inset-0 z-[130] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="w-full max-w-md rounded-2xl border border-red-500/30 bg-slate-50 dark:bg-gray-900 p-5 shadow-2xl">
                        <h3 className="text-xl font-bold text-red-300 mb-2">{t('login.bannedTitle')}</h3>
                        <p className="text-sm text-slate-900 dark:text-white/85">{banModal.message}</p>
                        <p className="text-sm text-slate-900 dark:text-white/80 mt-2">
                            {banModal.bannedUntil
                                ? t('login.bannedUntil', { date: new Date(banModal.bannedUntil).toLocaleString() })
                                : t('login.bannedDurationUnknown')}
                        </p>
                        <p className="text-xs text-slate-900 dark:text-white/60 mt-3">
